...
4. **CLAUDE.md & README**: rewrite the "two modes" section into "discover → adapt → validate → autofix". (The `mode` concept was subsequently removed entirely — see §13 — so no mode label survives anywhere.)
5. **Validator schema:** ships as `validator/1.0.0`. Validation is uniform; the validator takes no `--mode` and emits no `mode` field (the concept was removed entirely — see §13).
...
- **The `mode` concept, entirely.** With one uniform code path and no consumers, "mode" earned its removal: the validator's `--mode` flag and `mode` JSON output field are gone (no validation rule ever branched on them — the field was vestigial output left over from the retired side-by-side validation branch), and the `fresh` / `straight-swap` labels are replaced everywhere by the plain fact **"is MaxSDK present?"**. The behavioral fork is unchanged — when Max is present the integrator rewrites the game's call sites and inits with MAX mediation, otherwise it does a standalone install with `null` mediation — it is simply never called a "mode". This supersedes §8 steps 4–5, which had kept the `mode` field.
