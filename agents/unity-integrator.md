...
1. **`MeticaAdService.cs`** — render `$PLUGIN_DIR/scripts/templates/standalone/MeticaAdService.cs.tmpl`. Substitution proceeds in this order:
   ...
   **`__REVENUE_DELIVERY__` (revenue-callback delivery mode)** — substitute the setting that goes once, before `MeticaSdk.Initialize`. Fill it **only when a 3PA forwarder is being wired** (the "3PA revenue forwarders" patch pass fires) **and** the installed SDK is **≥ 2.4.2** (the `CallbackDelivery` / `RevenueCallbackDelivery` API exists only then — read the version from the compat-checker's `metica_sdk` row); otherwise substitute the **empty string** (the token line collapses):
   - No MaxSDK, or MaxSDK at its native-thread default (no `MaxSdk.InvokeEventsOnUnityMainThread = true` in the project) → `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.NativeThread;` — the generated/relocated forwarder is native-safe.
   - MaxSDK with `MaxSdk.InvokeEventsOnUnityMainThread = true` (detected in the Step 2 Max-callsite scan, before Step 5 removes it) → `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.UnityMainThread;` — matches the main-thread contract the relocated forwarder was written for.

   This is the matched mode the validator's `threepa_forwarder_in_revenue_paid` rule expects; applying it here means the rule sees an explicit, correct setting and doesn't ADVISORY. When the token is left empty because the SDK is `< 2.4.2`, surface the upgrade recommendation in the Step 7 report instead.
...
| **3PA revenue forwarders** | a 3PA provider detected (Step 2.5 Signal 3) | Populate each used format's named `On<Format>RevenuePaid` handler with the matching forwarder call, **one per detected provider**. **Prefer relocating the game's own existing forwarder calls** (found during the Max-callsite scan / callback-subscription rewrite) — they are already version-correct for the game's installed 3PA SDK versions — into the handler; only fall back to a fresh call (canonical shapes in `references/3pa-forwarders.md`) when the game had none. Forward from `OnAdRevenuePaid` **only** — never `OnAdHidden` or a dismissal hook (that loses click-through revenue; the validator FAILs it). No App-Open handler (App Open is `drop` per the TSV). **When this pass fires, also fill the `__REVENUE_DELIVERY__` token** with the matched delivery mode per rule (c) above (SDK ≥ 2.4.2 only). |
