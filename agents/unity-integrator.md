...
1. **Attach `MeticaAdService`** to a `DontDestroyOnLoad` GameObject in the first scene — name the **same GameObject** that hosts the existing wrapper (the `Attach target` from discovery, e.g. the object carrying `AdsManager` in `<scene>`). Until it's attached, `MeticaAdService.Instance` is null at runtime and every ad call silently no-ops (a wasted build/QA session). The integrator does **not** edit scene/prefab YAML itself — it names the exact GameObject so the user wires it in the Editor; the validator's `metica_service_attached` rule then verifies it.
2. **Set the real credentials in the Inspector** on that component (`apiKey` / `appId` / `maxSdkKey` / the ad-unit ids — all `[SerializeField]`). The validator's `metica_service_credentials_set` rule FAILs while they're empty or still the placeholder defaults.
