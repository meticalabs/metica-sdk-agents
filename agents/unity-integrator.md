...
When any 3PA forwarder is generated, also emit one ADVISORY line in the Step 7 report. On SDK ≥ 2.4.2 it recommends setting `MeticaAds.RevenueCallbackDelivery` before `MeticaSdk.Initialize`; the mode depends on whether MaxSDK is present:
- **MaxSDK present** — match the game's MaxSDK threading: MAX default (native callbacks) → `CallbackDelivery.NativeThread` (keeps fullscreen revenue from being lost on app-close-mid-ad; the handler then runs off the Unity main thread, so keep it thread-safe and don't wrap the forwarder in a main-thread dispatcher); `MaxSdk.InvokeEventsOnUnityMainThread = true` → `CallbackDelivery.UnityMainThread` (keeps a main-thread-written forwarder correct, but click-through-no-return / app-close-mid-ad scenarios can still lose events, as under MAX).
- **No MaxSDK** — `CallbackDelivery.NativeThread` is the loss-resistant default; the generated handler forwards on the native thread and is native-safe.

Example line (MaxSDK-present, MAX default): *"3PA forwarders (Adjust / Firebase / AppMetrica / AppsFlyer) generated inside `OnAdRevenuePaid`. Set `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.NativeThread` before `MeticaSdk.Initialize` (matches your MaxSDK native-thread callbacks; keeps fullscreen revenue from being lost on app-close-mid-ad — the handler runs off the Unity main thread, so keep it thread-safe)."*
