...
| **Banner/MRec setter ordering** | `SetBanner*` / `SetMrec*` calls migrated from the game (or generated) | Emit setters **after** the matching `Create*` call, never before — a setter on a not-yet-created `adUnitId` silently no-ops. Order is: subscribe callbacks → `CreateBanner`/`CreateMrec` → `SetBanner*`/`SetMrec*` → `Load*` → `Show*`. On **SDK ≥ 2.4.2 (Android)** banner/MRec creation and loading are separated, so a `Load`/`Show` must follow a `Create` for the same id — never collapse them or emit a show before the create (the template's `Init<Format>` already does `Create` → `Load`, with `Show` as a separate game-triggered call). For interstitial/rewarded `SetInterstitial*` / `SetRewardedAd*`, re-apply after each load (pre-creation calls are dropped per the TSV). Keeps the validator's `*_setter_after_create` rules green. |
