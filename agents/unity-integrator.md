...
1. **`MeticaAdService.cs`** — render `$PLUGIN_DIR/scripts/templates/standalone/MeticaAdService.cs.tmpl`. Substitution proceeds in this order:
   ...
   **`__REVENUE_DELIVERY__` (revenue-callback delivery mode)** — the single place the delivery mode is set, filled here at render time (the later "3PA revenue forwarders" pass doesn't touch it). Always fill it when the effective SDK (`$TARGET_SDK`) is **≥ 2.4.2**; otherwise **delete the whole token line** (the `CallbackDelivery` API doesn't exist pre-2.4.2 — recommend upgrading in Step 7). `MeticaAds` / `CallbackDelivery` resolve under the existing `using Metica.Ads;`. Value, matched to the game's MAX threading:
   - MaxSDK with `MaxSdk.InvokeEventsOnUnityMainThread` set `true` (either spelling; read the assigned value, whitespace-tolerant — a non-literal that can't be resolved to `false` → treat as main-thread and flag in Step 7) → `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.UnityMainThread;`.
   - Otherwise (MAX at its native-thread default, or no MaxSDK) → `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.NativeThread;`.
