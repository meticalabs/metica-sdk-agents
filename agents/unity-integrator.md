...
1. **`MeticaAdService.cs`** — render `$PLUGIN_DIR/scripts/templates/standalone/MeticaAdService.cs.tmpl`. Substitution proceeds in this order:
   ...
   **`__REVENUE_DELIVERY__` (revenue-callback delivery mode)** — this token is the **single** place the delivery mode is set, filled here at render time from the discovery signals (the later "3PA revenue forwarders" patch pass does **not** touch it — by then the token is already substituted). Fill it **only when a 3PA provider was detected** (Step 2.5 Signal 3, so the forwarder pass will wire a forwarder) **and** the effective SDK the integrated project will run — `$TARGET_SDK` on an upgrade, the freshly-installed version otherwise, **not** the originally-detected one — is **≥ 2.4.2** (the `CallbackDelivery` / `RevenueCallbackDelivery` API exists only then). Otherwise **delete the whole token line** (don't leave a blank, whitespace-only line). `MeticaAds` and the `CallbackDelivery` enum resolve under the template's existing `using Metica.Ads;`, so no extra `using` is needed. Value:
   - No MaxSDK, or MaxSDK at its native-thread default (no `MaxSdk.InvokeEventsOnUnityMainThread` assigned `true` in the project) → `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.NativeThread;` — the generated/relocated forwarder is native-safe.
   - MaxSDK with `MaxSdk.InvokeEventsOnUnityMainThread` set to `true` (from the Step 2 Max-callsite scan, before Step 5 removes it — read the **assigned value**, whitespace-tolerant, not a fixed `= true` substring) → `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.UnityMainThread;` — matches the main-thread contract the relocated forwarder was written for. When the flag is assigned a **non-literal** (a variable / const / config field) that can't be statically resolved to `false`, treat it as potentially main-thread: choose `UnityMainThread` (the safe choice for a main-thread-written forwarder) and flag it in the Step 7 report for the user to confirm.

   This is the matched mode the validator's `threepa_forwarder_in_revenue_paid` rule expects; setting it here means the rule sees an explicit, correct value and doesn't ADVISORY. When the line is omitted because `$TARGET_SDK` is `< 2.4.2` (only possible when a caller pins an older `--version`), surface the upgrade recommendation in the Step 7 report instead.
...
| **3PA revenue forwarders** | a 3PA provider detected (Step 2.5 Signal 3) | Populate each used format's named `On<Format>RevenuePaid` handler with the matching forwarder call, **one per detected provider**. **Prefer relocating the game's own existing forwarder calls** (found during the Max-callsite scan / callback-subscription rewrite) — they are already version-correct for the game's installed 3PA SDK versions — into the handler; only fall back to a fresh call (canonical shapes in `references/3pa-forwarders.md`) when the game had none. Forward from `OnAdRevenuePaid` **only** — never `OnAdHidden` or a dismissal hook (that loses click-through revenue; the validator FAILs it). No App-Open handler (App Open is `drop` per the TSV). The matched `MeticaAds.RevenueCallbackDelivery` for these forwarders is set once via the `__REVENUE_DELIVERY__` token at render time (rule (c) above), **not** in this pass. |
...
When any 3PA forwarder is generated, also report the revenue-callback delivery outcome in Step 7. On **SDK ≥ 2.4.2** codegen has already applied `MeticaAds.RevenueCallbackDelivery` (the `__REVENUE_DELIVERY__` token) before `MeticaSdk.Initialize`, matched to the project:
- **MaxSDK present** — matched to the game's MaxSDK threading: MAX default (native callbacks) → `CallbackDelivery.NativeThread` (keeps fullscreen revenue from being lost on app-close-mid-ad; the handler runs off the Unity main thread, so keep it thread-safe and don't wrap the forwarder in a main-thread dispatcher); `MaxSdk.InvokeEventsOnUnityMainThread = true` → `CallbackDelivery.UnityMainThread` (keeps a main-thread-written forwarder correct, but click-through-no-return / app-close-mid-ad scenarios can still lose events, as under MAX).
- **No MaxSDK** — `CallbackDelivery.NativeThread`, the loss-resistant default; the generated handler forwards on the native thread and is native-safe.

On **`$TARGET_SDK` < 2.4.2** the token line was omitted (the API doesn't exist) — recommend upgrading to ≥ 2.4.2 to gain it. Example line (MaxSDK-present, MAX default): *"3PA forwarders (Adjust / Firebase / AppMetrica / AppsFlyer) wired inside `OnAdRevenuePaid`. Applied `MeticaAds.RevenueCallbackDelivery = CallbackDelivery.NativeThread` before `MeticaSdk.Initialize` (matches your MaxSDK native-thread callbacks; keeps fullscreen revenue from being lost on app-close-mid-ad — the handler runs off the Unity main thread, so keep any code you add to it thread-safe)."*
