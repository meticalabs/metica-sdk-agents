...
| `default_adunit_is_applovin` | surface | A Metica-dedicated ad unit is configured as the app's default/requested id — the requested id passes through for holdout users, so the control arm runs on the wrong unit and the trial-vs-holdout comparison is invalid. Which AppLovin ad unit is the game's real one is a config decision, not a line edit; the integrator's own codegen reuses the game's existing Max ad-unit ids, so a FAIL is hand-rolled. Surface the id's definition (`file:line`) → the SDK call it reaches, and stop. |
...
| `no_load_during_show` | surface | A `Load*` runs while a fullscreen ad is showing (reachable from an interstitial/rewarded mid-show handler) — a known SDK-level issue. Rescheduling the load (moving it to `OnAdHidden`/`OnAdShowFailed`, or gating it behind an is-showing flag) is a control-flow fix, not a line edit — surface the load `file:line` + the mid-show entry it's reachable from, and stop. The integrator's codegen loads only from `OnInitialized` / `OnAdHidden` / `OnAdShowFailed` and the `OnAdLoadFailed` backoff retry (the rule's exempt same-format shape), so a FAIL is hand-rolled. |
| `banner_recreate_delay` | surface | A timed banner recreate delay that differs between the arms of a SmartFloors group branch or the `UseMetica` arms skews the trial-vs-holdout comparison. Whether and where to delay banner recreation is game logic, not a line edit — surface the branch (`file:line`) → the divergent delay and stop. |
...
`*_show_ready_guard`, `*_show_after_init`, `*_load_after_init`, `load_dedup_flag_wedge`, `format_path_symmetry`, `holdout_matches_baseline`, `dead_code_signal`, `metica_deprecated_api`, and `revenue_callback_subscribed` are `ADVISORY`, and `compiles_cleanly` is `WARN` when the compile is skipped (no Unity located / `METICA_SKIP_COMPILE=1`) or could not complete — none of these are `FAIL`, so they take no action and never affect status.
