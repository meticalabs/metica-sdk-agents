...
| Credential-storage pattern | how the wrapper holds keys/ad-unit ids — `[SerializeField] private string` (Inspector/prefab-set, the studio standard), a const, a config asset. The generated `MeticaAdService` mirrors it (Step 5); `[SerializeField]` is the template default. | `Credential storage` |
| Scripts-folder layout | is the wrapper's directory **flat** (sibling `.cs`, no code subfolders) or nested? Drives whether the adapter lands beside the wrapper or in a `Metica/` subfolder (Step 2.5 adapter-folder pick). | `Scripts layout` |
| Wrapper attach target | grep `Assets/**/*.unity` + `Assets/**/*.prefab` for the wrapper component's `.meta` GUID; record the GameObject (`m_Name`) and scene/prefab it lives on. The Step 7 report tells the user to attach `MeticaAdService` to that **same** GameObject. | `Attach target` |
...
5. **What do the load-success / load-failed handlers do beyond logging?** If the host's `On<Format>AdLoaded` / `On<Format>AdFailed` handlers swap with or fall back to **another ad network** (e.g. on banner load: hide AdMob + show this banner; on fail: hide this + show AdMob) or set a readiness flag, that behaviour must be **ported into the generated `On<Format>LoadSuccess` / `On<Format>LoadFailed` named handlers**, not just logged — otherwise the migration silently drops the game's fallback. Because it is game-specific cross-network wiring (the other network is whatever the host uses, not necessarily AdMob), **surface the proposed port in the Step 3 plan for confirmation** rather than inventing it.
...
- **Adapter folder pick** — **if discovery detected a wrapper, place the adapter beside it**; otherwise `ls "$PROJECT/Assets/"`: if `Assets/_Project/Scripts/` exists use that, else `Assets/Game/Scripts/`, else `Assets/Scripts/`. **Match the host's layout** (`Scripts layout` from discovery): when the chosen directory is **flat** (sibling `.cs`, no code subfolders) **and** codegen emits a single `MeticaAdService.cs`, write the file directly into that directory — do **not** create a one-file `Metica/` subfolder. Only nest under `<dir>/Metica/` when the host already organises code into subfolders or when you split into per-format `MeticaAdService.<Format>.cs` files. (E.g. flat `Assets/AdsPlugin/Scripts/` → `Assets/AdsPlugin/Scripts/MeticaAdService.cs`, not `…/Scripts/Metica/MeticaAdService.cs`.)
...
1. **`MeticaAdService.cs`** — render `$PLUGIN_DIR/scripts/templates/standalone/MeticaAdService.cs.tmpl`: apply the namespace transform (below), **drop the `// @fmt-begin:<fmt>`…`// @fmt-end:<fmt>` region for every format NOT in `$FORMATS`**, and substitute the credential/id tokens — which the template declares as **`[SerializeField]` fields** so the studio can set or rotate them in the Inspector / build pipeline (the studio-standard pattern): `__METICA_API_KEY__` / `__METICA_APP_ID__` / `__MAX_SDK_KEY__` (escaped as above) into the credential field defaults, the **game's existing Max ad unit IDs** into each used format's `_<fmt>AdUnitId` field default (per the migration guide they pass through unchanged), `__USER_ID__` (verbatim) into the config, and `__MEDIATION__` → `new MeticaMediationInfo(MeticaMediationInfo.MeticaMediationType.MAX, _maxSdkKey)` — referencing the `_maxSdkKey` **field**, not an inline string (note: `MeticaMediationType` is a **nested** enum inside `MeticaMediationInfo`, so it **must** be qualified `MeticaMediationInfo.MeticaMediationType.MAX` — the bare form from the docs page does not compile; the SDK source and the canonical demo win over the docs). If discovery found the host's existing wrapper storing credentials/ids a different way, **mirror that storage pattern**; otherwise keep the `[SerializeField]` defaults — never inline literals in the `Initialize` call. The result is a single `MonoBehaviour` that sets privacy + `MeticaSdk.SetLogEnabled(true)` (both **before** `MeticaSdk.Initialize`, same file) and calls `MeticaSdk.Initialize(config, <mediation>, OnInitialized)` with the **named `OnInitialized` method** (not a lambda) that logs `SmartFloors` and wires up each used format; it exposes `LoadInterstitial`/`ShowInterstitial`, `LoadRewarded`/`ShowRewarded`, `ShowBanner`/`HideBanner`, `ShowMrec`/`HideMrec` — these (and `Initialize`) must be **called on the Unity main thread** (the SDK captures the `SynchronizationContext` at the call site); when rewriting call sites that fire from a CMP/consent or other off-main callback, marshal them to the main thread. **File layout:** by default write one `$ADAPTER_FOLDER/MeticaAdService.cs`; for a larger project you may split each `@fmt` region into a `$ADAPTER_FOLDER/MeticaAdService.<Format>.cs` `partial class MeticaAdService` to match the game's conventions (the validator is content-based and passes either way).
...
| **Mirror host load/fail fallback** | host load-success/failed handlers do more than log — a network swap/fallback or a readiness flag (per-format discovery Q5) | Port that behaviour into the generated `On<Format>LoadSuccess` / `On<Format>LoadFailed` named handlers — set the readiness flag, and call the host's fallback network's hide/show (whatever network the host actually uses, guarded by a null check). **Generalise** — never assume AdMob; use the network discovery found. Confirm the port in the Step 3 plan; do not invent a network the host doesn't use. |
...
| `no_missing_script_orphans` | surface | A missing-script GameObject is Editor cleanup, not a code edit — surface the scene/prefab + GameObject name (and that it's likely a deleted Metica/ads script) and stop. |
| `metica_service_attached` | surface | The component isn't attached to any scene/prefab — an Editor step. Surface the exact GameObject to attach it to (the `Attach target` from discovery) and stop; the integrator does not edit scene/prefab YAML. |
| `metica_service_credentials_set` | surface \| prompt | Credentials are unset/placeholder in the Inspector. Prompt for the real values if the user wants them baked into the prefab; otherwise surface which fields to set in the Editor and stop. The integrator does not write prefab YAML. |
...
**Required actions (promote these like a compat BLOCK — never bury them in a "manual steps" footnote).** A clean compile + validator PASS does **not** mean the integration is live; these are the steps the code can't do for the user:

1. **Attach `MeticaAdService`** to a `DontDestroyOnLoad` GameObject in the first scene — name the **same GameObject** that hosts the existing wrapper (the `Attach target` from discovery, e.g. the object carrying `AdsManager` in `<scene>`). Until it's attached, the component is never instantiated, so its `Start()` → `Initialize()` never runs and MeticaSDK is never initialized — every ad call silently no-ops (a wasted build/QA session). The integrator does **not** edit scene/prefab YAML itself — it names the exact GameObject so the user wires it in the Editor; the validator's `metica_service_attached` rule then verifies it.
2. **Set the real credentials in the Inspector** on that component (`_apiKey` / `_appId` / `_maxSdkKey` / the ad-unit-id fields — all `[SerializeField]`). The validator's `metica_service_credentials_set` rule FAILs while they're empty or still the placeholder defaults.
...
4. **Manual steps remaining** — set the real user identity for correct cross-session attribution (`null`/empty is valid — Metica auto-generates a per-device id), and choose the `SetHasUserConsent`/`SetDoNotSell` values per compliance posture (or wire your CMP — see `privacy_before_init`).
