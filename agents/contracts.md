...
- `no_missing_script_orphans` — **scene/prefab, FAIL-capable**. Grep `.unity`/`.prefab` for `m_Script: {fileID: 0,` (orphaned MonoBehaviour from a deleted/renamed class); report the GameObject `m_Name`. **FAIL** when integration-related (e.g. a leftover `MeticaManager`), **ADVISORY** for unrelated project orphans.
- `metica_service_attached` — **scene/prefab, FAIL-capable**. The generated service `.cs`'s `.meta` guid must be referenced by a build scene (`EditorBuildSettings.asset`) or prefab; **FAIL** if nothing instantiates it (runtime `Instance` is null, no ads load), **ADVISORY** if the meta/scene list can't be read.
