...
- `no_missing_script_orphans` — **scene/prefab, FAIL-capable**. Grep `.unity`/`.prefab` for `m_Script: {fileID: 0` (orphaned MonoBehaviour from a deleted/renamed class; matches both the bare `{fileID: 0}` and `{fileID: 0, guid: …}` forms); report the GameObject `m_Name`. **FAIL** when integration-related (e.g. a leftover `MeticaManager`), **ADVISORY** for unrelated project orphans.
- `metica_service_attached` — **scene/prefab, FAIL-capable**. The generated service `.cs`'s `.meta` guid must be referenced by a build scene (`EditorBuildSettings.asset`) or prefab; **FAIL** if nothing instantiates it (the component never runs `Start()`/`Initialize()`, so the SDK is never initialized — no ads load), **ADVISORY** if the meta/scene list can't be read.
- `metica_service_credentials_set` — **scene/prefab, FAIL-capable**. Read the `MeticaAdService` component's serialized `_apiKey`/`_appId`/`_maxSdkKey` from the prefab/scene YAML; **FAIL** when empty, a placeholder default, or a format mismatch (api key = 32-char hex); **ADVISORY** when the serialized values can't be confidently parsed. Prefab-side counterpart of `placeholder_ids_replaced`.
...
- **Behavioral checks** split into two groups by the level they emit:
  - **Gating** (`*_reload_on_hidden`, `placement_ids_match`, `smartfloors_analytics_only`, `banner_setter_after_create` / `mrec_setter_after_create`, `interstitial_setter_after_create` / `rewarded_setter_after_create`, `threepa_forwarder_in_revenue_paid`, `callbacks_fire_on_every_path`, `init_callback_all_paths`, `retry_ownership`, `sdk_calls_on_main_thread`, `no_missing_script_orphans`, `metica_service_attached`, `metica_service_credentials_set`): emit `PASS`/`FAIL`, so they gate `status` like any other check — a `FAIL` triggers the autofix loop, a `PASS` is trusted. **Exception:** `smartfloors_analytics_only`, `threepa_forwarder_in_revenue_paid`, `callbacks_fire_on_every_path`, `init_callback_all_paths`, `retry_ownership`, `sdk_calls_on_main_thread`, and the scene/prefab rules (`no_missing_script_orphans`, `metica_service_attached`, `metica_service_credentials_set`) FAILs are **`surface`-class** (removing group-aware ad logic, relocating a 3PA forwarder, fixing a missed-callback / init / retry / off-main-thread path, or attaching/configuring a component in the Editor is game-logic / Editor work, not a line edit — surfaced for a human); the setter-ordering FAILs are autofix-eligible only when create and setter sit in the same method (else `surface`). A behavioral FAIL with non-empty `unresolved` (the validator was unsure) is also **`surface`-class** — never fed to the autofix loop; a human decides.
