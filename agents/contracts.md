...
```json
{
  ...
  "checks": [
    ...
    { "id": "metica_sdk",        "detected": "2.4.3",       "required": ">=2.4.0",  "level": "PASS",    "hint": "" }
  ]
}
```
...
each verdict. It shells out for the Unity batch compile (`scripts/compile-check.sh`) behind
`compiles_cleanly` and a read-only `git show` of the `pre-metica-integration` tag behind
`holdout_matches_baseline`. Its semantic verdicts gate `status` like any other check.
...
- `init_count` — exactly one full-init call site (`MeticaSdk.Initialize(` or `MeticaSdk.InitializeAsync(`, counted together; `InitializeAnalytics(` doesn't count). Init rules generally cover both forms — for the async form, init completes when the awaited task resolves, so "downstream of `OnInitialized`" and "after the `await`" are equivalent gates.
...
- `adunit_routing_unreliable` — **behavioral, FAIL-capable** (project-wide). A returned `MeticaAd.adUnitId` is not a reliable routing signal — SmartFloors serves trial Metica-dedicated ad units regardless of the requested id, and a holdout ad unit can appear in the trial route as a connection-issue fallback (see `references/smartfloors-user-groups.md`). **FAIL** when a guard branches on a returned `ad.adUnitId == / != <configured>` (or a stored copy) and that branch gates an ad-control decision (ad-unit selection/branch, a `Load*`/`Show*` gate, or an ad-lifecycle state switch) **or drops callback processing** (an early return that skips revenue forwarding, reward granting, or lifecycle handling on id mismatch — a MAX idiom that silently drops revenue under Metica; key per-format routing on the ad format / callback source, not id equality). PASS when the returned id is only logged/attributed or never compared; group-aware branching (`response.SmartFloors.UserGroup` / `IsForcedHoldout`) is the sanctioned pattern and is **not** flagged. `ADVISORY` (with `unresolved`) when the flow can't be traced — never a blind FAIL.
- `load_callback_fields_unreliable` — **behavioral, FAIL-capable** (project-wide). On SmartFloors trial, loading is decoupled from the game: the load callback reports whichever internal instance resolved first (usually the zero-floor fallback) and the SDK afterwards rotates inventory silently with no further game events, so the callback's `MeticaAd` payload fields (`revenue`, `adUnitId`, `networkName`) are biased/stale/substituted on trial (see `references/smartfloors-user-groups.md`) — a gate built on them misbehaves only for trial users. **FAIL** when a load-callback payload field is persisted (or used inline) in a conditional that gates showing or loading (e.g. an eCPM threshold on stored load-callback revenue, or a `networkName` branch selecting load/show behavior); an `adUnitId`-equality guard against a configured id (or a stored copy) stays under `adunit_routing_unreliable` (never emitted under both). **ADVISORY** when the payload `adUnitId` is persisted and fed back into subsequent MeticaSdk calls (`Load*` / `Show*` / `Is<Format>Ready` with a callback-derived id rather than the configured publisher id) — currently neutralized by the SDK on trial but version-fragile. **PASS** when payload fields are only logged/attributed/forwarded to revenue trackers or never read. `ADVISORY` (with `unresolved`) when the flow can't be traced — never a blind FAIL.
- `default_adunit_is_applovin` — **behavioral, FAIL-capable** (project-wide). The ad-unit ids the app configures and passes to `Load*`/`Show*`/`Create*` (including the default arm of an id-selection fallback) must be the game's own AppLovin MAX ad units; Metica-dedicated ad units are provisioned for the SDK's internal trial routing and are never configured app-side — the requested id passes through for holdout users (see `references/smartfloors-user-groups.md`), so a Metica-dedicated default puts the control arm on the wrong unit and invalidates the trial-vs-holdout comparison. Judged by provenance, not value: a constant/config key or nearby comment marking the id as Metica's, or a `UseMetica`/SmartFloors-group branch selecting a different id per arm (every arm must request the same publisher unit). **FAIL** when a Metica-marked id is the id an SDK call requests; **PASS** when every configured id is the game's own MAX unit; a stored callback-derived id fed back into SDK calls stays under `load_callback_fields_unreliable` (never emitted under both). `ADVISORY` (with `unresolved`) when provenance can't be judged — never a blind FAIL.
- `smartfloors_group_branch_complete` — **behavioral, FAIL-capable** (project-wide). Group-aware ad control is sanctioned, but both SmartFloors groups must reach ads (holdout waterfall + trial single call). **FAIL** when a branch on `response.SmartFloors.UserGroup` / `IsForcedHoldout` gates ad loading/showing and leaves a group with no reachable `Load*`/`Show*` path (e.g. `if (IsForcedHoldout) StartAdLoading();` with no `else` starves trial — the no-ads-for-trial regression the removed analytics-only rule guarded). PASS when every group's branch reaches an ad-load path; not fired when the group is only logged/attributed. `ADVISORY` (with `unresolved`) when branches can't be traced. Checks branch completeness, not which strategy each group uses.
- `holdout_matches_baseline` — **behavioral, ADVISORY** (project-wide). The holdout group is the experiment's control and must reproduce the game's **baseline** ad behavior — the non-Metica implementation (see `references/smartfloors-user-groups.md`); the trial (Metica) branch is expected to differ and is never compared. Requires a holdout branch to judge — a branch on `response.SmartFloors.UserGroup`/`IsForcedHoldout` (or a stored copy); a single N/A PASS row when no such branch exists. Baseline sources, in order: a non-Metica arm still in the code (Max call sites behind a `MeticaAdService.UseMetica` switch — the `UseMetica == false` arm is a baseline source, never the holdout branch — or a dedicated Max wrapper file), else the pre-Metica logic via the `pre-metica-integration` git tag (read-only `git -C "$PROJECT" show`; a missing repo or tag means no baseline). Compares the holdout branch against it — load timing/cadence (artificial delays, retry/backoff shape), reload-on-hidden, show gating, callback handling, revenue forwarding, placement/ad-unit usage — and surfaces each divergence (a drifted holdout invalidates the trial-vs-holdout comparison). `evidence` entries cite working-tree files only; the tagged baseline is referenced in `reasoning`/`detail` as `pre-metica-integration:<path>:<line>`, never as an `evidence` entry. A lead, not a proven defect — never FAIL. PASS when holdout mirrors the baseline, or when no baseline is visible (no non-Metica arm, no tag). A baseline-divergent banner recreate delay stays under `banner_recreate_delay` (never emitted under both).
- `load_dedup_flag_wedge` — **behavioral, ADVISORY** (project-wide). Flags a self-managed ad state flag (`isLoading` / `isShow`) gating `Load`/`Show` that isn't cleared on every terminal event (load fail, show fail, hidden): redundant (the SDK dedups) and wedge-prone (a missed terminal callback leaves it stuck, blocking all later load/show/reload/retry). Recommendation only; never FAIL, never gates `status`. A show-window guard cleared in both `OnAdHidden` and `OnAdShowFailed` is the sanctioned `no_load_during_show` shape — not flagged (load fail is not one of its terminals).
- `no_load_during_show` — **behavioral, FAIL-capable** (project-wide). MeticaSDK mishandles a `Load*` issued while a fullscreen ad is displaying (it disrupted the in-flight show in a shipped game), so loads must stay out of the show window (`Show*`/`OnAdShowSuccess` → that ad's `OnAdHidden`/`OnAdShowFailed`). Version-gated: when `references/metica-sdk-migration.md` records the SDK release fixing load-during-show and the installed version is at/above it, a single PASS row citing that entry. **FAIL** when a `Load*` (any format) is provably reachable during a fullscreen show — inside an interstitial/rewarded mid-show handler (`OnAdShowSuccess`, `OnAdClicked`, `OnAdRevenuePaid`, `OnAdRewarded`, or a leftover MAX `OnAdDisplayedEvent`) or a chain rooted in one; banner/MRec lifecycle handlers are never mid-show entries. **ADVISORY** when a load path can coincide with a show but overlap isn't provable (periodic/scheduled loader, user-driven load) and no is-showing guard exists — the recommended guard is set on show and cleared in both `OnAdHidden` and `OnAdShowFailed`; the canonical same-format `OnAdLoadFailed` backoff retry is exempt (that format has no ad to show, so it cannot overlap its own show window; cross-format overlap is accepted as unprovable noise, not flagged). **PASS** when every load path runs outside the show window (initial loads from `OnInitialized`, reloads from `OnAdHidden`/`OnAdShowFailed`, the exempt retry) or sits behind an is-showing guard — a single N/A PASS row when no load is reachable from any show window, a cited guard chain (≥2 evidence) when the PASS rests on a guard. `ADVISORY` (with `unresolved`) when the flow can't be traced — never a blind FAIL.
- `banner_recreate_delay` — **behavioral, FAIL-capable**. A timed delay between banner teardown and recreation (`DestroyBanner`/`HideBanner` → `WaitForSeconds`/`Invoke`/`Task.Delay`/timer → `CreateBanner`/`LoadBanner`/`ShowBanner`) changes the effective banner refresh cadence and skews the trial-vs-holdout comparison (see `references/smartfloors-user-groups.md`) when it differs between the compared arms (a shipped game delayed banner recreation on its Metica route only). Branch axes: a SmartFloors group branch (`response.SmartFloors.UserGroup`/`IsForcedHoldout`, or a stored copy) and/or the `MeticaAdService.UseMetica` arms. **FAIL** when both arms of such a branch are visible and the delay differs between them — present in only one arm, or a different duration per arm; **ADVISORY** when a timed delay precedes banner (re)creation and no in-code comparison arm exists (surfaced with "confirm the delay matches the pre-Metica/holdout banner cadence"); **PASS** with no artificial delay or a delay provably identical across arms; a single N/A PASS row when the project never tears down and recreates a banner (canonical `Create` once → `Show`, SDK auto-refreshes). Frame-boundary yields (`yield return null`, `WaitForEndOfFrame`) are not flagged. `ADVISORY` (with `unresolved`) when the flow can't be traced — never a blind FAIL.
- `banner_setter_after_create` / `mrec_setter_after_create` — **behavioral, FAIL-capable**. Every `MeticaSdk.Ads.SetBanner*` / `SetMrec*` for an `adUnitId` must be preceded on every path by `CreateBanner`/`CreateMrec` for the same id; a pre-create setter silently no-ops (wrapper warns `BANNER not found for adUnitId`), the bug that disabled `adaptive_banner=true` in a shipped game. FAIL when a setter precedes its create; ADVISORY (with `unresolved`) when ordering can't be traced. On **SDK ≥ 2.4.2 (Android)** banner/MRec creation and display are separate steps (canonical lifecycle `Create` → `Show`; the SDK loads/auto-refreshes, so an explicit `Load` is only for manual refresh), so the same create-precedence covers a banner/MRec `Show`/`Load`: FAIL when one isn't preceded on every path by a `Create` for the same id. On **installed** SDK `≥ 2.45.0` (read from `Assets/MeticaSdk/Runtime/Sdk/MeticaSdk.cs` → `Version`) the native bridge buffers pre-create **value setters** only — `SetBannerExtraParameter` / `SetBannerLocalExtraParameter` / `SetBannerCustomData` and `SetMrec*` equivalents — and applies them on `Create*`: PASS those six with `detail: "buffered pre-create — 2.45.0+ native (see references/metica-sdk-migration.md)"`. FAIL stands on every version for pre-create `SetBanner/MrecPlacement`, `SetBannerBackgroundColor`, and `SetBannerWidth` (not buffered), and for a `Show`/`Load` without a preceding `Create`.
...
- `threepa_forwarder_in_revenue_paid` — **behavioral, FAIL-capable**. When a 3PA SDK (Adjust, Firebase Analytics, AppsFlyer, AppMetrica) is present, its ad-revenue forwarding call must live inside a `MeticaAdsCallbacks.<Format>.OnAdRevenuePaid` handler. **FAIL** when found inside `OnAdHidden` / a dismissal hook / any other lifecycle hook (revenue then reports only on dismissal — click-through users lose every event). **ADVISORY** when correctly inside `OnAdRevenuePaid` (on SDK < 2.4.2 dispatch rides Unity's main thread via `SynchronizationContext.Post`, so click-through-no-return may still lose events), or when `MeticaAds.RevenueCallbackDelivery` on a ≥2.4.2 project doesn't match the game's MAX threading per the delivery-mode rule in `references/3pa-forwarders.md` (or, when MAX threading isn't observable, when no explicit mode is set before `Initialize`), or when it is set only after `Initialize`. Under `CallbackDelivery.NativeThread` the handler runs off the Unity main thread and two more FAILs apply: a handler whose call chain touches a Unity-main-thread-only API (the throw silently aborts the rest of the handler), and a forwarder wrapped in a main-thread marshal inside the handler (the player loop is paused during fullscreen display, so the posted report is lost on app-death-mid-ad — report first on the native thread, marshal Unity work after).
...
- `init_gated_on_cmp` — **behavioral, FAIL-capable**. When a consent management platform (Google UMP, AppLovin `MaxCmpService`, or a host-custom consent gate) is present, `MeticaSdk.Initialize` + preceding privacy setters must run **only after the CMP resolves consent** (from its completion callback), never unconditionally from `Awake()`/`Start()` or a bootstrap ahead of it — initializing first ships wrong/empty consent to MeticaSDK and the mediated ad networks (a compliance violation). **FAIL** when a CMP is present and `Initialize` is provably reachable before the CMP callback fires; **PASS** when init is downstream of the CMP callback, or when no CMP is detected (single PASS row); **ADVISORY** (with `unresolved`) when a CMP is present but the consent→init ordering can't be traced. Composes with — does not duplicate — `init_callback_all_paths` (callback fires on every path) and `sdk_calls_on_main_thread` (CMP callback marshals init to the main thread).
- `init_reentry_guarded` — **behavioral, FAIL-capable**. The init + callback-subscription path must not be re-executable at runtime: each re-run re-subscribes every `MeticaAdsCallbacks.* +=` handler, and every subsequent callback fires once per subscription (duplicate revenue reports, duplicate reward grants). Distinct from `init_count` (textual call-site count — this rule judges **runtime re-entry through one site**: scene-reloaded bootstrap `Start()`, retry/focus/login handlers, multiple runtime paths). **FAIL** when re-entry is provable and the subscriptions have no re-entry guard (idempotency flag before any `+=`, or `-=` before `+=`); **PASS** when guarded or provably single-shot; **ADVISORY** (with `unresolved`) when reachability can't be traced.
...
- `max_api_use_metica` — emits **one row per match** for every Max-API call site whose replacement is documented in `references/max-metica-api-map.tsv` with `kind=rename` or `kind=signature-change`. The scan covers all non-exempt namespaces — `MaxSdk.`, `MaxSdkBase.`, `MaxSdkCallbacks.`, `MaxCmpService.` (not just `MaxSdk.`; `MaxSdkUtils.*` is exempt). `level` is **FAIL** in any file that also references `MeticaSdk.` (Metica owns the live `AppLovinSdk` instance, so the direct `MaxSdk.*` call is dead) and **ADVISORY** in a pure-Max file (e.g. a side-by-side `AdManager.cs` wrapper). **Exemption:** a Max call site reachable only under the false branch of a `MeticaAdService.UseMetica` switch (the integrator's `MAX_MODE=switchable` output) is the sanctioned coexistence path — not flagged; the validator verifies the gate by reading the code (the integrator's Task prompt may state `MAX_MODE=switchable` as context, never trusted alone), and an ungated call keeps the FAIL model. `detail` carries the suggested replacement. Single **PASS** row when no matches anywhere.
- `max_api_unsupported` — companion rule for `kind=drop` rows (no MeticaSdk equivalent — App Open Ads, segment targeting, expanded/collapsed banner callbacks, etc.). Same FAIL/ADVISORY/PASS model **and the same switchable-gate exemption**. `detail` advises removing the call or isolating it behind a Max-only code path. `MaxSdkUtils.*` is exempt (stateless helpers, mix-safe).
- `metica_deprecated_api` — **ADVISORY**. Use of an **obsoleted** or **signature-changed** MeticaSDK symbol per `references/metica-sdk-migration.md` for the installed SDK version (seed: `MeticaSmartFloors.IsSuccess` → `IsForcedHoldout` / `UserGroup`). Obsolete symbols still compile (`[Obsolete]` warns, so `compiles_cleanly` misses them); this is the post-upgrade confirmation that no stale usage remains. `detail` carries the replacement. Single **PASS** row when no matches.
...
- `validator.status == FAIL` → run the **integrator-owned autofix loop** (classify each FAIL as `autofix` / `prompt` / `surface`, apply edits with an anchor re-check, log to `$PROJECT/.metica-integration.log`, re-validate; **max 3 iterations**). A `compiles_cleanly` FAIL is **autofix-eligible when the cited file was created or edited by this run** (per the checkpoint log's `WROTE`/`EDITED` lines; the re-validate re-runs the compile, so a wrong fix is caught within the iteration cap) and `surface`-class otherwise (printed verbatim with `file:line`, not auto-edited). Only when the loop cannot clear all FAILs (a `surface`-class FAIL remains, or 3 iterations are exhausted) print the rollback command and exit non-zero. Never auto-rollback — rollback stays a *hint*. The validator itself remains **read-only**; the integrator owns all edits and prompts. See `agents/unity-integrator.md` Step 6.5.
...
- **Behavioral checks** split into two groups by the level they emit:
  - **Gating** (`*_reload_on_hidden`, `placement_ids_match`, `adunit_routing_unreliable`, `load_callback_fields_unreliable`, `default_adunit_is_applovin`, `smartfloors_group_branch_complete`, `no_load_during_show`, `banner_recreate_delay`, `banner_setter_after_create` / `mrec_setter_after_create`, `interstitial_setter_after_create` / `rewarded_setter_after_create`, `threepa_forwarder_in_revenue_paid`, `callbacks_fire_on_every_path`, `init_callback_all_paths`, `init_gated_on_cmp`, `init_reentry_guarded`, `retry_ownership`, `sdk_calls_on_main_thread`): emit `PASS`/`FAIL`, so they gate `status` like any other check — a `FAIL` triggers the autofix loop, a `PASS` is trusted. **Exception:** `adunit_routing_unreliable`, `load_callback_fields_unreliable`, `default_adunit_is_applovin`, `smartfloors_group_branch_complete`, `no_load_during_show`, `banner_recreate_delay`, `threepa_forwarder_in_revenue_paid`, `callbacks_fire_on_every_path`, `init_callback_all_paths`, `init_gated_on_cmp`, `retry_ownership`, and `sdk_calls_on_main_thread` FAILs are **`surface`-class** (removing ad-unit-id routing, removing a payload-field show/load gate, swapping a Metica-dedicated configured ad unit back to the game's AppLovin unit, fixing a group branch that starves a group of ads, rescheduling a mid-show load, removing a group-asymmetric banner recreate delay, relocating a 3PA forwarder, or fixing a missed-callback / init / consent-ordering / retry / off-main-thread path is game-logic redesign, not a line edit — surfaced for a human); the setter-ordering FAILs are autofix-eligible only when create and setter sit in the same method (else `surface`). A behavioral FAIL with non-empty `unresolved` (the validator was unsure) is also **`surface`-class** — never fed to the autofix loop; a human decides.
  - **ADVISORY-only** (`*_show_ready_guard`, `*_show_after_init`, `*_load_after_init`, `load_dedup_flag_wedge`, `format_path_symmetry`, `holdout_matches_baseline`, `dead_code_signal`, `metica_deprecated_api`): always emit `level: ADVISORY`, so per the Status rule above they surface in the report but never affect `status` and never trigger the autofix loop. (`retry_ownership` is ADVISORY for the dead-counter case, FAIL only when auto-retry is disabled with no client retry.)
