...
- **Different / Metica-dedicated ad units in the trial group → do NOT flag.** The trial group is served Metica-dedicated ad units regardless of the ad-unit id the app requested, so the callback's `MeticaAd.adUnitId` frequently differs from the requested id — that divergence is **by design**, not a config error. An occasional **holdout ad unit appearing in the trial route** is also expected: it's a connection-issue fallback, not a routing bug. So do **not** analyse or attribute the trial route by ad-unit id (adunit-based routing is unreliable here); **attribute the revenue comparison by group, not by ad-unit id.** See `references/smartfloors-user-groups.md` for the sanctioned group-aware pattern and the expected group differences.
