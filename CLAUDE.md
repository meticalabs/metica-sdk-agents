...
**The integrator's three modes** are the central branch. `scripts/detect-mode.sh` looks for three MaxSDK signals (`Assets/MaxSdk/` folder, `MaxSdk.Initialize(` symbol, AppLovin manifest); two-of-three → MaxSDK present, else **fresh**. When Max is present, the integrator sub-divides on the remote-config provider detected in Step 2.5: no provider → **straight-swap** (replace Max in the game's *direct* call sites with MeticaSDK, no router; never touch a dedicated Max-wrapper file), a real provider → **side-by-side** (full router stack that A/B-tests Max vs Metica, never touches Max code — call-site rewrites are only *proposed*). `detect-mode.sh` reports only Max presence (`fresh`/`side-by-side`); `straight-swap` is derived in integrator/validator prose (see `agents/contracts.md`). **Generated Metica code is split per ad format in every mode**: a `MeticaAdService` orchestrator (init + privacy, in one file) plus per-format objects (`MeticaInterstitialAd`/`MeticaRewardedAd`/`MeticaBannerAd`) that own their callbacks, auto-reload-on-hidden (interstitial/rewarded), and `IsReady`-guarded show. Fresh/straight-swap use the standalone templates (`scripts/templates/standalone/`, no `IAdService`); side-by-side uses `scripts/templates/sidebyside/` where the orchestrator implements `IAdService` so the router can swap it. Everything lands under `Assets/Scripts/Metica/` (`namespace Metica.AbTest` by default).
